// Command Virenmonitoring-cli is a CLI + REPL for the Virenmonitoring SDK,
...
const prompt = "virenmonitoring"

// entitiesHelp is the space-separated entity list shown by /help.
const entitiesHelp = "dataset_metadata virus_monitoring"
...
func usage(out io.Writer) {
	...
	fmt.Fprintln(out, "Env:      VIRENMONITORING_APIKEY (api key), VIRENMONITORING_BASE (base url override)")
...
func run(args []string, in io.Reader, out, errOut io.Writer) int {
	...
	// Configure from the environment: VIRENMONITORING_APIKEY carries the API key and
	// VIRENMONITORING_BASE optionally overrides the API base URL (e.g. production).
	// Both injectable by a secrets vault. Unset -> nil config defaults.
	var opts map[string]any
	if apikey := os.Getenv("VIRENMONITORING_APIKEY"); apikey != "" {
		opts = map[string]any{"apikey": apikey}
	}
	if base := os.Getenv("VIRENMONITORING_BASE"); base != "" {
		if opts == nil {
			opts = map[string]any{}
		}
		opts["base"] = base
	}
	client := sdk.NewVirenmonitoringSDK(opts)
