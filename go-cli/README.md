...
boru-driven command-line client **and** interactive REPL for the Virenmonitoring
...
```sh
# 1. Build a native binary (-> dist/<os>-<arch>/virenmonitoring-cli)
make build

# 2. See usage (words, entities, env vars)
./virenmonitoring-cli --help

# 3. Provide credentials once, via the environment
export VIRENMONITORING_APIKEY=sk_live_xxx

# 4. Each command line is ONE boru expression, run against the API:
./virenmonitoring-cli list dataset_metadata
./virenmonitoring-cli list virus_monitoring

# 5. Override the API base URL for a single call
VIRENMONITORING_BASE=https://api.example.com ./virenmonitoring-cli list dataset_metadata

# 6. No arguments -> interactive REPL
./virenmonitoring-cli
virenmonitoring> list dataset_metadata
virenmonitoring> /quit
```
...
1. **Build the binary.** From this `go-cli/` directory:
   ...
   ```sh
   make build          # -> dist/<os>-<arch>/virenmonitoring-cli
   ```
...
3. **Run a query.** Evaluate an boru expression against the API (or run with no
   ...
   ```sh
   ./dist/*/virenmonitoring-cli list dataset_metadata
   ```
...
Run with no arguments to open a REPL (prompt `virenmonitoring>`). Each line is
evaluated as its own boru expression:

```text
$ ./virenmonitoring-cli
virenmonitoring> list dataset_metadata
virenmonitoring> /help
virenmonitoring> /quit
```
...
- `<entity>` is a bareword, auto-quoted as an boru atom (e.g. `dataset_metadata`).
...
| Target | Result |
|--------|--------|
| `make build` | Native binary at `dist/<os>-<arch>/virenmonitoring-cli`. |
...
(`./virenmonitoring-cli <expr>`) and interactively (the REPL), and expressions compose the
