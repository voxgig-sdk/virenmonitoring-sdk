...
[MCP](https://modelcontextprotocol.io) server exposing the Virenmonitoring SDK as
two agent tools — `virenmonitoring_list` and `virenmonitoring_load` — built on the
...
```sh
# 1. Build a native binary (-> dist/<os>-<arch>/virenmonitoring-mcp)
make build

# 2. Provide credentials via the environment
export VIRENMONITORING_APIKEY=sk_live_xxx

# 3a. Install into Claude Code over stdio (most common)
claude mcp add --scope user virenmonitoring \
  -- /absolute/path/to/virenmonitoring-mcp -transport stdio

# 3b. …or run a shared HTTP server instead
./virenmonitoring-mcp -transport http -addr :8080
```

Tool-call arguments (what an agent sends):

```jsonc
// virenmonitoring_list: first page of records
{ "entity": "dataset_metadata" }
{ "entity": "dataset_metadata", "query": { } }

// virenmonitoring_load: one record by id
{ "entity": "dataset_metadata", "query": { "id": 1 } }
```
...
1. **Build** the server from this `go-mcp/` directory:
   ...
   ```sh
   make build          # -> dist/<os>-<arch>/virenmonitoring-mcp
   ```
...
3. **Install it into Claude Code** (stdio transport):
   ...
   ```sh
   claude mcp add --scope user virenmonitoring \
     -- "$PWD"/dist/*/virenmonitoring-mcp -transport stdio
   ```

4. **Restart Claude Code.** The `virenmonitoring_list` and `virenmonitoring_load` tools now appear
   in new sessions. Ask the agent to *"list dataset_metadata using virenmonitoring"*
   and it calls `virenmonitoring_list` with `{"entity":"dataset_metadata"}`.
...
```sh
export VIRENMONITORING_APIKEY=sk_live_xxx            # API key
export VIRENMONITORING_BASE=https://api.example.com  # optional: override the API base URL
```
...
### Call the `virenmonitoring_list` tool
...
### Call the `virenmonitoring_load` tool
...
| Tool | Args | Returns |
|------|------|---------|
| `virenmonitoring_list` | `entity` (required), `query` (optional map) | First page of records as JSON |
| `virenmonitoring_load` | `entity` (required), `query` = `{id:N}` | Single record as JSON |
...
| Variable | Purpose |
|----------|---------|
| `VIRENMONITORING_APIKEY` | API key sent with every request. |
| `VIRENMONITORING_BASE` | Optional override of the API base URL. |

### Entities

The 2 entities valid as the `entity` argument:

dataset_metadata | virus_monitoring

### Smoke test via HTTP (raw JSON-RPC)

```sh
./virenmonitoring-mcp -transport http -addr :18080 &
...
curl -sN -X POST http://localhost:18080 \
  ...
  -d '{"jsonrpc":"2.0","id":2,"method":"tools/call","params":{"name":"virenmonitoring_load","arguments":{"entity":"dataset_metadata","query":{"id":1}}}}'
```
