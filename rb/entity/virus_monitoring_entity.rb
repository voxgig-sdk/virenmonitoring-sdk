...
class VirusMonitoringEntity
  ...
  def make
    opts = @_entopts.dup
    VirusMonitoringEntity.new(@_client, opts)
  end

  def data_set(args)
    if args
      @_data = VirenmonitoringHelpers.to_map(VoxgigStruct.clone(args)) || {}
      @_utility.feature_hook.call(@_entctx, "SetData")
    end
  end

  # @return [VirusMonitoring, Hash] the current VirusMonitoring data
  def data_get
  ...
  def match_set(args)
    if args
      @_match = VirenmonitoringHelpers.to_map(VoxgigStruct.clone(args)) || {}
      @_utility.feature_hook.call(@_entctx, "SetMatch")
    end
  end

  # @return [Hash] the current match filter (any subset of VirusMonitoring fields)
  def match_get
  ...
  #
  # @param reqmatch [VirusMonitoringListMatch, Hash, nil] match filter (any subset of
  #   VirusMonitoring fields); defaults to nil, treated as an empty match that lists all.
  # @param ctrl [Object, nil] optional per-call control
  # @return [Array<VirusMonitoring>, Array] the matching VirusMonitoring items; raises VirenmonitoringError on failure
