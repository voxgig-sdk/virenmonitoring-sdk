...
class DatasetMetadataDirectTest < Minitest::Test
  def test_direct_list_dataset_metadata
    setup = dataset_metadata_direct_setup([
      { "id" => "direct01" },
      { "id" => "direct02" },
    ])
    _should_skip, _reason = Runner.is_control_skipped("direct", "direct-list-dataset_metadata", setup[:live] ? "live" : "unit")
    if _should_skip
...
def dataset_metadata_direct_setup(mockres)
