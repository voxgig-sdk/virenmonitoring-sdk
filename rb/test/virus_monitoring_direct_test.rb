...
class VirusMonitoringDirectTest < Minitest::Test
  def test_direct_list_virus_monitoring
    setup = virus_monitoring_direct_setup([
      { "id" => "direct01" },
      { "id" => "direct02" },
    ])
    _should_skip, _reason = Runner.is_control_skipped("direct", "direct-list-virus_monitoring", setup[:live] ? "live" : "unit")
    if _should_skip
...
def virus_monitoring_direct_setup(mockres)
