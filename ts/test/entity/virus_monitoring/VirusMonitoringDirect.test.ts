...
describe('VirusMonitoringDirect', async () => {
  ...
  test('direct-exists', async () => {
    const sdk = new VirenmonitoringSDK({
      system: { fetch: async () => ({}) }
    })
  ...
  test('direct-list-virus_monitoring', async (t: any) => {
    const setup = directSetup([{ id: 'direct01' }, { id: 'direct02' }])
    if (maybeSkipControl(t, 'direct', 'direct-list-virus_monitoring', setup.live)) return
...
function directSetup(mockres?: any) {
  ...
  if (live) {
    ...
    let idmap: any = env['VIRENMONITORING_TEST_VIRUS_MONITORING_ENTID']
