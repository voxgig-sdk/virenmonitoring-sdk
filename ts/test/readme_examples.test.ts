...
//     (\`new VirenmonitoringSDK(...)\` / \`VirenmonitoringSDK.test(...)\` become
//     \`VirenmonitoringSDK.test({ entity: ... })\`), transpiled to JS, and run
...
const SDK_NAME = 'VirenmonitoringSDK'
...
const TEST_SEED = {"entity":{"dataset_metadata":{"test01":{"id":"test01"}},"virus_monitoring":{"test01":{"id":"test01"}}}}
...
// rewrite real client construction (\`new VirenmonitoringSDK(...)\` and any
// \`VirenmonitoringSDK.test(...)\`) to a seeded test-mode client, and give
...
function rewriteForRun(code: string): string {
  ...
  out = out.replace(/new\s+VirenmonitoringSDK\s*\([^)]*\)/g, () => SEEDED_CTOR)
  out = out.replace(/VirenmonitoringSDK\.test\s*\([^)]*\)/g, () => SEEDED_CTOR)
