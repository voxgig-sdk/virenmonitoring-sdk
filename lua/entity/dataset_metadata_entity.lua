...
local DatasetMetadataEntity = {}
DatasetMetadataEntity.__index = DatasetMetadataEntity


function DatasetMetadataEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, DatasetMetadataEntity)
  self._name = "dataset_metadata"
  self._client = client
...
function DatasetMetadataEntity:get_name()
  return self._name
end


function DatasetMetadataEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return DatasetMetadataEntity.new(self._client, opts)
end
...
function DatasetMetadataEntity:mark_deleted()
  self._deleted = true
end


function DatasetMetadataEntity:deleted()
  return true == self._deleted
end


function DatasetMetadataEntity:data_set(args)
  if args ~= nil then
...
function DatasetMetadataEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function DatasetMetadataEntity:match_set(args)
  if args ~= nil then
...
function DatasetMetadataEntity:match_get()
...
function DatasetMetadataEntity:stream(action, args, callopts)
...
function DatasetMetadataEntity:_run_op(ctx, post_done)
  local utility = self._utility
