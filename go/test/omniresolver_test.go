...
import (
	...
	omni "github.com/voxgig-sdk/virenmonitoring-sdk/go/test/omni"
...
func omniProvider(client any) *omni.Provider {
	provider := &omni.Provider{
		...
		Errify: func(err any) any {
			if sdkerr, is := err.(*sdk.VirenmonitoringError); is {
				out := map[string]any{
					"name":    "VirenmonitoringError",
...
func omniCtx(arg any, client *sdk.VirenmonitoringSDK, utility *sdk.Utility) *sdk.Context {
	...
	if provider, is := ctxmap["client"].(*omni.Provider); is {
		if live, is := defproviderclient(provider).(*sdk.VirenmonitoringSDK); is {
			client = live
