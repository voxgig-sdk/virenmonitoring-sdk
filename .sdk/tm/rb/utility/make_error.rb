...
module VirenmonitoringUtilities
  MakeError = ->(ctx, err) {
    if ctx.nil?
      ...
      ctx = VirenmonitoringContext.new({}, nil)
    end
    op = ctx.op || VirenmonitoringOperation.new({})
    opname = op.name
    opname = "unknown operation" if opname.empty? || opname == "_"

    result = ctx.result || VirenmonitoringResult.new({})
    result.ok = false

    err = result.err if err.nil?
    err = ctx.make_error("unknown", "unknown error") if err.nil?

    errmsg = err.is_a?(VirenmonitoringError) ? err.msg : err.to_s
    ...
    sdk_err = VirenmonitoringError.new("", msg, ctx)
    ...
    sdk_err.code = err.code if err.is_a?(VirenmonitoringError)
