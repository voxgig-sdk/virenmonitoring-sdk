...
module VirenmonitoringUtilities
  MakeRequest = ->(ctx) {
    ...
    spec = ctx.spec
    utility = ctx.utility
    response = VirenmonitoringResponse.new({})
    ...
    if fetch_err
      ...
      response = VirenmonitoringResponse.new({ "err" => ctx.make_error("request_no_response", "response: undefined") })
    elsif fetched.is_a?(Hash)
      response = VirenmonitoringResponse.new(fetched)
    else
