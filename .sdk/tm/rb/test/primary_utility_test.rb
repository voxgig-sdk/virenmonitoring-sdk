...
class PrimaryUtilityTest < Minitest::Test
  ...
  def setup
    @runner = VirenmonitoringOmni.make_runner(TEST_JSON_FILE, VirenmonitoringSDK.test(nil, nil))
  ...
  def err_from_map(m)
    ...
    VirenmonitoringError.new(m["code"] || "", msg)
  end
  ...
  def test_make_point_basic
    ...
    subject = lambda do |ctx|
      ...
      err.nil? ? point : VirenmonitoringOmni.errify(err)
    end
  ...
  def test_operator_basic
    subject = lambda do |vin|
      vin = {} unless vin.is_a?(Hash)
      op = VirenmonitoringOperation.new(vin)
      {
        "entity" => op.entity,
        "name" => op.name,
        "input" => op.input,
