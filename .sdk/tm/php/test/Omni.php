...
//    only - a typed VirenmonitoringContext there would make every ctx
...
final class VirenmonitoringOmni
{
    ...
    public static function ctx_from_map($ctxmap, $client, $utility)
    {
        ...
        $ctx = new VirenmonitoringContext($clean, null);
        ...
        if (isset($clean['spec']) && is_array($clean['spec'])) {
            $ctx->spec = new VirenmonitoringSpec($clean['spec']);
        }

        if (isset($clean['result']) && is_array($clean['result'])) {
            $ctx->result = new VirenmonitoringResult($clean['result']);
            if (isset($clean['result']['err']) && is_array($clean['result']['err'])) {
                $msg = $clean['result']['err']['message'] ?? '';
                $ctx->result->err = new VirenmonitoringError('', is_string($msg) ? $msg : '');
            }
        }

        if (isset($clean['response']) && is_array($clean['response'])) {
            $ctx->response = new VirenmonitoringResponse($clean['response']);
    ...
    public static function err_from_map($m): ?VirenmonitoringError
    {
        if (!is_array($m)) {
            return null;
        }
        ...
        $code = $m['code'] ?? '';
        return new VirenmonitoringError(is_string($code) ? $code : '', $msg);
    }
