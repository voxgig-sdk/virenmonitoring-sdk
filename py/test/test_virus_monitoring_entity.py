...
class TestVirusMonitoringEntity:

    def test_should_create_instance(self):
        testsdk = VirenmonitoringSDK.test(None, None)
        ent = testsdk.VirusMonitoring(None)
    ...
    def test_should_stream(self):
        ...
        base = VirenmonitoringSDK.test(seed, None)
        seen = list(base.VirusMonitoring(None).stream("list", None, None))
        assert len(seen) == 3

        # Inbound: streaming active -> yields each item from the feature.
        from virenmonitoring_sdk.config import shared_config
        cfg = shared_config()
        if isinstance(cfg.get("feature"), dict) and "streaming" in cfg["feature"]:
            sdk = VirenmonitoringSDK.test(
                seed, {"feature": {"streaming": {"active": True}}})
            got = []
            for item in sdk.VirusMonitoring(None).stream("list", None, None):
    ...
    def test_should_run_basic_flow(self):
        setup = _virus_monitoring_basic_setup(None)
        # Per-op sdk-test-control.json skip — basic test exercises a flow with
        # multiple ops; skipping any one skips the whole flow (steps depend
        # on each other).
        _live = setup.get("live", False)
        for _op in ["list"]:
            _skip, _reason = runner.is_control_skipped("entityOp", "virus_monitoring." + _op, "live" if _live else "unit")
            if _skip:
        ...
        if setup.get("synthetic_only"):
            pytest.skip("live entity test uses synthetic IDs from fixture — "
                        "set VIRENMONITORING_TEST_VIRUS_MONITORING_ENTID JSON to run live")
        client = setup["client"]

        # Bootstrap entity data from existing test data.
        virus_monitoring_ref01_data_raw = vs.items(helpers.to_map(
            vs.getpath(setup["data"], "existing.virus_monitoring")))
        virus_monitoring_ref01_data = None
        if len(virus_monitoring_ref01_data_raw) > 0:
...
def _virus_monitoring_basic_setup(extra):
    runner.load_env_local()

    entity_data_file = os.path.join(_TEST_DIR, "../../.sdk/test/entity/virus_monitoring/VirusMonitoringTestData.json")
    ...
    idmap_resolved = helpers.to_map(
        env.get("VIRENMONITORING_TEST_VIRUS_MONITORING_ENTID"))
    ...
    if env.get("VIRENMONITORING_TEST_LIVE") == "TRUE":
        ...
        client = VirenmonitoringSDK(helpers.to_map(merged_opts))

    _live = env.get("VIRENMONITORING_TEST_LIVE") == "TRUE"
    return {
        "client": client,
        "data": entity_data,
        "idmap": idmap_resolved,
        "env": env,
        "explain": env.get("VIRENMONITORING_TEST_EXPLAIN") == "TRUE",
        "live": _live,
