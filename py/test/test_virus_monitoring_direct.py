...
class TestVirusMonitoringDirect:

    def test_should_direct_list_virus_monitoring(self):
        setup = _virus_monitoring_direct_setup([
            {"id": "direct01"},
            {"id": "direct02"},
        ])
        _skip, _reason = runner.is_control_skipped("direct", "direct-list-virus_monitoring", "live" if setup["live"] else "unit")
        if _skip:
...
def _virus_monitoring_direct_setup(mockres):
