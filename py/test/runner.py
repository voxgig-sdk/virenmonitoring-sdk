# Virenmonitoring SDK test runner
...
class VirenmonitoringTestRunner:
    _env = {}
    ...
    def load_env_local():
        ...
        for line in content.splitlines():
            ...
            VirenmonitoringTestRunner._env[key] = val

    @staticmethod
    def getenv(key):
        val = VirenmonitoringTestRunner._env.get(key)
        if val is not None:
            return val
    ...
    @staticmethod
    def env_override(m):
        live = VirenmonitoringTestRunner.getenv("VIRENMONITORING_TEST_LIVE")
        override = VirenmonitoringTestRunner.getenv("VIRENMONITORING_TEST_OVERRIDE")

        if live == "TRUE" or override == "TRUE":
            for key in list(m.keys()):
                envval = VirenmonitoringTestRunner.getenv(key)
        ...
        explain = VirenmonitoringTestRunner.getenv("VIRENMONITORING_TEST_EXPLAIN")
        if explain is not None and explain != "":
            m["VIRENMONITORING_TEST_EXPLAIN"] = explain
    ...
    def load_test_control():
        ...
        """
        if VirenmonitoringTestRunner._test_control is not None:
            return VirenmonitoringTestRunner._test_control
        ctrl_path = os.path.join(os.path.dirname(__file__), "sdk-test-control.json")
        try:
            with open(ctrl_path, "r") as f:
                VirenmonitoringTestRunner._test_control = json.load(f)
        except (FileNotFoundError, IOError, ValueError):
            VirenmonitoringTestRunner._test_control = {
                "version": 1,
                "test": {"skip": {
    ...
    def is_control_skipped(kind, name, mode):
        ...
        ctrl = VirenmonitoringTestRunner.load_test_control()
...
def load_env_local():
    VirenmonitoringTestRunner.load_env_local()


def env_override(m):
    return VirenmonitoringTestRunner.env_override(m)


def entity_data(v):
    return VirenmonitoringTestRunner.entity_data(v)


def entity_list_to_data(lst):
    return VirenmonitoringTestRunner.entity_list_to_data(lst)


def is_control_skipped(kind, name, mode):
    return VirenmonitoringTestRunner.is_control_skipped(kind, name, mode)


def load_test_control():
    return VirenmonitoringTestRunner.load_test_control()


def live_client_options():
    return VirenmonitoringTestRunner.live_client_options()


def live_delay_ms():
    return VirenmonitoringTestRunner.live_delay_ms()
