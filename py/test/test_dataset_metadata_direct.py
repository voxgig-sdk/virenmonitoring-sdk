...
class TestDatasetMetadataDirect:

    def test_should_direct_list_dataset_metadata(self):
        setup = _dataset_metadata_direct_setup([
            {"id": "direct01"},
            {"id": "direct02"},
        ])
        _skip, _reason = runner.is_control_skipped("direct", "direct-list-dataset_metadata", "live" if setup["live"] else "unit")
        if _skip:
...
def _dataset_metadata_direct_setup(mockres):
