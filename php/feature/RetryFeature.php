...
// Virenmonitoring SDK retry feature
...
class VirenmonitoringRetryFeature extends VirenmonitoringBaseFeature
{
    ...
    public function init(VirenmonitoringContext $ctx, array $options): void
    {
        ...
        $utility->fetcher = function (VirenmonitoringContext $ctx2, string $url, array $fetchdef) use ($inner): array {
            return $this->_with_retry($ctx2, $url, $fetchdef, $inner);
        };
    }

    private function _with_retry(VirenmonitoringContext $ctx, string $url, array $fetchdef, callable $inner): array
    {
        $opts = $this->options ?? [];
    ...
    private function _track(VirenmonitoringContext $_ctx, int $attempt, mixed $res, mixed $err, float $wait): void
    {
        $client = $this->client;
