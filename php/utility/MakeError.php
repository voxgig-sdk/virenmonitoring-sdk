...
class VirenmonitoringMakeError
{
    public static function call(?VirenmonitoringContext $ctx, mixed $err): mixed
    {
        if ($ctx === null) {
            require_once __DIR__ . '/../core/Context.php';
            $ctx = new VirenmonitoringContext([], null);
        }
        $op = $ctx->op ?? new VirenmonitoringOperation([]);
        $opname = $op->name;
        if ($opname === '' || $opname === '_') {
            $opname = 'unknown operation';
        }

        $result = $ctx->result ?? new VirenmonitoringResult([]);
        $result->ok = false;
        ...
        $errmsg = ($err instanceof VirenmonitoringError) ? $err->msg : (string)$err;
        ...
        $sdk_err = new VirenmonitoringError('', $msg, $ctx);
        $sdk_err->result = ($ctx->utility->clean)($ctx, $result);
        $sdk_err->spec = ($ctx->utility->clean)($ctx, $spec);
        if ($err instanceof VirenmonitoringError) {
