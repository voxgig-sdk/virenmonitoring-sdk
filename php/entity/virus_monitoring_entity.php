...
// Virenmonitoring SDK VirusMonitoring entity
...
class VirusMonitoringEntity
{
    ...
    {
        $opts = $this->_entopts;
        return new VirusMonitoringEntity($this->_client, $opts);
    }

    /**
     * @param VirusMonitoring|array $args VirusMonitoring data (assoc-array) to store.
     */
    public function data_set($args): void
    {
        if ($args) {
            $this->_data = VirenmonitoringHelpers::to_map(Struct::clone($args)) ?? [];
            ($this->_utility->feature_hook)($this->_entctx, "SetData");
        }
    }

    /**
     * @return VirusMonitoring|array The current VirusMonitoring data as an assoc-array.
     */
    ...
    /**
     * @param array $args Match filter (any subset of VirusMonitoring fields).
     */
    public function match_set($args): void
    {
        if ($args) {
            $this->_match = VirenmonitoringHelpers::to_map(Struct::clone($args)) ?? [];
            ($this->_utility->feature_hook)($this->_entctx, "SetMatch");
        }
    }

    /**
     * @return array The current match filter (any subset of VirusMonitoring fields).
     */
    ...
    /**
     * List VirusMonitoring items matching the given filter.
     *
     * @param VirusMonitoringListMatch|array|null $reqmatch Match filter (any subset
     *   of VirusMonitoring fields) as an assoc-array; VirusMonitoringListMatch names the shape.
     * @param mixed $ctrl Optional per-call control overrides.
     * @return VirusMonitoring[]|array A list of VirusMonitoring items as assoc-arrays at
     *   the SDK boundary; throws VirenmonitoringError on failure (item-5 convention).
     */
